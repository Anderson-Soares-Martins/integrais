...
const calculateTrapezoid = (points: Point[]) => {
  if (points.length !== 2) {
    throw new Error("Trapezoid rule requires exactly 2 points.");
  }
...
const calculateCompositeTrapezoid = (points: Point[]) => {
...
export default function Trapezio() {
  ...
  const estimateErrorTrapezoid = (points: Point[]) => {
    const n = points.length - 1;
    const h = (points[n].x - points[0].x) / n;
    const fPrimePrimeMax = Math.max(
      ...points.map((point) => evaluateExpression(expressionError, point.x))
    );

    return Math.abs(
      -(((points[n].x - points[0].x) * h ** 2) / 12) * fPrimePrimeMax
    );
  };
  ...
  const handleCalculate = () => {
    if (points.length < 2) {
      alert("Adicione pelo menos dois pontos para a Regra do Trapézio.");
      return;
    }
    ...
    try {
      const integral =
        points.length === 2
          ? calculateTrapezoid(points)
          : calculateCompositeTrapezoid(points);
      setResult(integral);

      if (expressionError) {
        const errorEstimate = estimateErrorTrapezoid(points);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24 text-black">
      <div className="text-center">
        ...
        <h2 className="text-2xl">Regra do Trapézio</h2>
        <button
